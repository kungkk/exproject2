app.controller("users_controller", function ($http, myFactory, $scope) {
    // #region GET API: /users/.json
    $scope.get = function () {
        $http({
    ...
    if (configs.initial_get.users == true) $scope.get();
    // #endregion
    ...
    $scope.delete = function () {
        ...
        if (window.confirm('Are you sure you want to delete?')) {
            $http({
                url: "/users?_=" + myFactory.date_time_now(),
                method: "DELETE",
                cache: false,
                ...
                myFactory.response_behavior(status, "DELETE", "USERS");
            }).error(function (data, status, headers, config) {
                myFactory.response_behavior(status, "DELETE", "USERS");
            }).finally(function () {
                $scope.loading = false;
    ...
    $scope.$on('on_users_json', function (event, args) {
        $scope.get();
    });
    // #endregion
